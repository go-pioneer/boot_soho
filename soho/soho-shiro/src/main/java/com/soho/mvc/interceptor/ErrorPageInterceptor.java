    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
        ...
        if (HttpUtils.isRetJson(request, deftConfig.getApiPrefix())) {
            if (response.getStatus() == 403) {
                RetData retData = new RetData(RetCode.BIZ_ERROR_STATUS, "您没有足够的权限访问该资源", new HashMap<>());
                HttpUtils.responseJsonData(response, retData);
            } else if (response.getStatus() == 404) {
                RetData retData = new RetData(RetCode.BIZ_ERROR_STATUS, "该访问资源不存在或已被移除", new HashMap<>());
                HttpUtils.responseJsonData(response, retData);
            } else if (response.getStatus() == 500) {
                RetData retData = new RetData(RetCode.BIZ_ERROR_STATUS, "访问资源发生未知异常错误", new HashMap<>());
                HttpUtils.responseJsonData(response, retData);
            }
        } else {
            if (response.getStatus() == 403) {
